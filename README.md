...
An automated, non-destructive Git backup solution specifically designed for the **Creality K1C (2025 Revision)**. Back up your complete Klipper configuration to GitHub with a single click from Fluidd or Mainsail.
...
* **Persistent Pathing:** Utilizes `/usr/data/printer_data/config` to ensure full persistence on the K1C 2025 revision architecture.
...
1. **GitHub Personal Access Token (PAT):** Generated on GitHub under `Settings` -> `Developer Settings` -> `Personal Access Tokens (Classic)` with full `repo` scope enabled.
2. **GitHub Repository:** An empty repository created on your GitHub account (e.g., `k1c-2025-klipper-backup`).
3. **gcode_shell_command:** Installed on your printer (via Klipper Helper Script / KIAUH).
...
2. **`git_backup.sh`** — A shell script that checks for file changes, commits them with a timestamp, and pushes to `main`:
   ```bash
   #!/bin/sh
...
2. Click the **`BACKUP_GITHUB`** button in the Macro panel.
